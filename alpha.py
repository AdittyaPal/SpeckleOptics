...
for i in range(1, 90, 1):
    ...
    for j in range(0,steps):
        guess, prev=chioa(guess, prev, mask, alpha[90-i], beta)
    sse[90-i]=calculateSSE(guess)
    print(i)

print("done")

fig, ax = plt.subplots()
ax.plot(alpha, sse, color='red', linewidth=0.5)
ax.set(xlabel=r'$\alpha$', ylabel='SSE in dB', title=r'SSE values varying $\alpha$')
plt.show()
