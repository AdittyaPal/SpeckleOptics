...
for i in range(0, 9, 1):
    ax.plot(step, sse[i,:], color=cmap[i], linewidth=0.6, label=f'(1, {cycle[i]-1})')
...
ax.set(xlabel='Number of Iterations in a cycle (K)', ylabel='SSE in dB', title='SSE values using CHIOA/HIOA varying the number of iterations in each cycle')
plt.show()
